...
def run_evaluation_engine(X_panel: pd.DataFrame, y_target: pd.Series, args: argparse.Namespace, out_dir: Path, suffix: str):
    ...
    for k, v in config.items():
        if k not in ["rmse", "mae", "mape", "status", "runtime_sec", "n_eval_points", "n_folds", "error_message", "n_raw_features", "n_trans_features", "n_sel_features", "n_model_used_vars", "regression_params"]:
            eval_config[k] = _parse_if_str(v)
    
    # Special handling for regression_kwargs which might be in regression_params
    if "regression_params" in config:
        eval_config["regression_kwargs"] = _parse_if_str(config["regression_params"])
    elif "regression_kwargs" not in eval_config:
        eval_config["regression_kwargs"] = {}

    logger.info(f"Evaluating MIDAS with config from {args.config_path}")
    ...
    if results and results[0]["status"] == "success":
        ...
        if not eval_df.empty:
            out_preds = out_dir / f"midas_eval_preds_{suffix}.csv"
            eval_df.to_csv(out_preds, index=False)
            logger.info(f"Saved predictions to {out_preds}")
        
        out_json = out_dir / f"midas_eval_results_{suffix}.json"
