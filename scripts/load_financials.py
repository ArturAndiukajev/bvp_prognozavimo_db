...
def ensure_series(conn, dataset_id: int, ticker: str, name: str) -> int:
    return conn.execute(text("""
        INSERT INTO series (dataset_id, key, country, frequency, transform, unit, name, meta)
        VALUES (:did, :key, 'Global', 'D', 'LEVEL', 'PRICE', :name, '{}'::jsonb)
        ON CONFLICT (dataset_id, key, country, frequency, transform)
        DO UPDATE SET name = EXCLUDED.name
        RETURNING id
    """), {"did": dataset_id, "key": ticker, "name": name}).scalar_one()
...
def copy_to_staging_and_merge(conn, rows: Iterable[Tuple[int, str, str, float, int]]):
    ...
    dbapi_conn = _get_psycopg2_connection(conn) # Renamed from _get_dbapi_connection
...
def ingest_one_ticker(ticker: str, details, mode: str, downloaded_at: datetime, vintage_at: datetime, stamp: str, tail_days_default: int) -> dict:
    ...
    with engine.begin() as conn:
        ...
        series_id = ensure_series(conn, dataset_id, ticker, name)
    ...
    with engine.begin() as conn:
        ...
        with Timer(f"Yahoo COPY {ticker}"):
            for i in range(0, len(rows), COPY_BATCH):
                ...
                copy_to_staging_and_merge(conn, chunk)
...
def main(mode: str = "initial", max_workers: int = 4):
    ...
    # Log individual result collection
    results = []
    ...
    with engine.begin() as conn:
        provider_id = ensure_provider(conn, "yahoo_finance", base_url="https://finance.yahoo.com")
        # log against the provider's first dataset or a sentinel dataset
